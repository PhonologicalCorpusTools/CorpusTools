...
def main():
    ...
    parser.add_argument('-y', '--type_or_token', default='token', help='For change in entropy FL: specifies whether entropy is based on type or token frequency.')
    parser.add_argument('-e', '--relative_fl', type=check_bool, default=False, help="If True, instead of reading the ")
    parser.add_argument('-t', '--tier', default='transcription', help="The attribute of Words to calculate FL over. Normally this will be the transcription, but it can also be the spelling or a user-specified tier.")
    ...
    if args.algorithm == 'minpair':
        if args.relative_fl:
            result = relative_minpair_fl(corpus, segpairs_or_segment, frequency_cutoff=args.frequency_cutoff, relative_count=bool(args.relative_count), distinguish_homophones=args.distinguish_homophones, tier=args.tier)
        else:
            result = minpair_fl(corpus, segpairs_or_segment, frequency_cutoff=args.frequency_cutoff, relative_count=bool(args.relative_count), distinguish_homophones=args.distinguish_homophones, tier=args.tier)
    elif args.algorithm == 'deltah':
        if args.relative_fl:
            result = relative_deltah_fl(corpus, segpairs_or_segment, frequency_cutoff=args.frequency_cutoff, type_or_token=args.type_or_token, tier=args.tier)
        else:
            result = deltah_fl(corpus, segpairs_or_segment, frequency_cutoff=args.frequency_cutoff, type_or_token=args.type_or_token, tier=args.tier)
    else:
