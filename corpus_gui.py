...
##        output_file_frame.grid(row=3, column=0)
    ...
    def about_acoustic_similarity(self):
        about_as = Toplevel()
        about_as.title('Acoustic similarity')
        desc_frame = LabelFrame(about_as, text='Brief description')
        ...
        source_frame = LabelFrame(about_as, text='Original sources')
        ...
        coder_frame = LabelFrame(about_as, text='Coded by')
    ...
    def process_as_queue(self,update):
        try:
            ...
            self.as_popup.after(100, lambda x=update:self.process_as_queue(update))

    def delete_as_results_table(self):

        #clean-up function
        if self.as_results_table is not None:
            self.as_results_table.destroy()
            self.as_results_table = None
            self.as_results.destroy()
        try:
    ...
    def show_as_result(self, result):
        self.as_results = Toplevel()
        self.as_results.protocol('WM_DELETE_WINDOW', self.delete_as_results_table)
        self.update_as_button.config(state=ACTIVE)
        self.as_results.title('Acoustic similarity results')
        ...
        self.as_results_table = MultiListbox(self.as_results,[('Directory 1',10),
                                                ('Directory 2',10),
                                                ('Representation', 10),
                                                ('Match function', 10),
                                                ('Minimum frequency',5),
        ...
        self.as_results_table.insert(END,[os.path.split(self.as_directory_one.get())[1],
        ...
        button_frame = Frame(self.as_results)
        print_button = Button(button_frame, text='Save results to file', command=self.print_as_results)
        print_button.grid(row=0, column=0)
        close_button = Button(button_frame, text='Close this table', command=self.delete_as_results_table)
        close_button.grid(row=0, column=1)
        button_frame.grid()

    def print_as_results(self):
