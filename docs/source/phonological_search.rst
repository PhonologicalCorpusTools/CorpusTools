...
6. **Additional Filters**: As of PCT 1.5.0, there are some extra options that can be added to phonological searches. These act as filters. If the filters are left blank, they are simply not applied and all results are returned. It is **VERY IMPORTANT** to realize that they act as *initial* filters to the corpus, not filters on the *results*. This is crucially relevant for negative searches. Specifically, the filter is applied to the corpus first, before the search is conducted. For example, if the minimum phoneme number is set to be 3, then only words that are at least three phonemes long are considered. Then, whatever actual search is requested is performed on the *filtered* corpus. For example, if the search is for words that begin with [m] in the example corpus, then only words that are already included as being three phonemes long will be included (e.g., [mɑtɑ]). For positive searches, the effects are cumulative and the order isn't relevant. But for negative searches, the filter is treated as a positive filter requirement, and then the negative search is performed. So if a negative search for words that begin with [m] is requested, along with a filter for only results that are at least three phonemes long, then only words that *are* longer than three phonemes AND *don't* begin with [m] will be returned (e.g., [nɑtɑ]). The alternative would be to first do a full positive search on the whole corpus, looking for words that begin with [m] and are longer than three phonemes, and then returning all words that do not match those criteria. The difference in the result is whether words that are shorter than three phonemes and don't begin with [m] are included (e.g., [ʃi] in the example corpus). The actual implementation of PCT will *not* include such words, whereas the alternative interpretation described above *would* include such words. The three filters are:
