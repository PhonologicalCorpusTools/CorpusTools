...
def test_freqalt(specified_test_corpus):
    ...
    result = calc_freq_of_alt(specified_test_corpus,'s','ʃ','edit_distance','type', max_rel = 2,
                                phono_align=True)
    assert(result==(8,2,0.25))

    result = calc_freq_of_alt(specified_test_corpus,'s','ʃ','edit_distance','token', max_rel = 4,
                                phono_align=True)
    assert(result==(8,3,0.375))
    ...
    result = calc_freq_of_alt(specified_test_corpus,'s','ʃ','phono_edit_distance','type', max_rel = 6,
                                phono_align=True)
    assert(result==(8,2,0.25))

    result = calc_freq_of_alt(specified_test_corpus,'s','ʃ','phono_edit_distance','token', max_rel = 20,
                                phono_align=True)
    assert(result==(8,3,0.375))
